...
def handle_conversation(msg):
    ...
    if not context:
        return "No valid datasets found. Please check the file paths and formats."

    if msg.lower() == "exit":
        return "Goodbye! I hope you have a good day!"

    start_time = time.time()

    response_type = is_greeting_or_how_are_you(msg)
    if response_type == "greeting":
        return "Hi there! 😊 I'm so glad to see you. How can I assist you today?"
    elif response_type == "how_are_you":
        mental_response = response_of_mental_state(msg)
        return "I'm doing great, thank you for asking! 😄 How are you doing? How is your mental health today?", mental_response
    
    result = chain.invoke({"context": context, "question": msg})
    emotional_response = add_emotional_tone(msg)
    
    context += f"\nUser: {msg}\nAI: {emotional_response}"
    
    conv_time = time.time() - start_time
    print(f"Response generated in {conv_time:.2f} seconds")
    
    return emotional_response + " " + result
